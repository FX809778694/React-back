...
export const addSubExtendData = (data) => {
  return () => {
    console.log(data)
    return api.addSubExtendData(data)
...
export const EditSubExtendData = (data, id) => {
  return () => {
    console.log(data)
    return api.editSubExtendData(data, id)
      .then(response => ({status: response.status}))
      .then(({status}) => {
        if (status === 200) {
          console.log('信息修改成功！')
...
export const deleteSubExtendData = (id) => {
  return () => {
    console.log(id)
    return api.deleteSubExtendData(id)
      .then(response => ({status: response.status}))
      .then(({status}) => {
        if (status === 204) {
