...
import {CONFIG_SEARCH_BOOST} from '../../../../../utils/routePath'
...
export default function TableData ({handleDelete, sorter, filByEnabled, ...props}) {
  const columns = [
    {
      ...
      key: 'id',
      render: (text, record) => (
        <Link to={`${CONFIG_SEARCH_BOOST}/id/${record.id}`}>{text}</Link>
      ),
      sorter: true,
      ...
      render: (text, record) => (
        <span style={{fontSize: 16}}>
          <Link to={`${CONFIG_SEARCH_BOOST}/modify/${record.id}`}>
            <Icon type="edit" title="编辑助推函数"/>
          </Link>
          {
            record.enabled !== 0 &&
            <a>
  ...
  return (
    <TableComponent label="助推函数列表" addLink={`${CONFIG_SEARCH_BOOST}/add`} columns={columns} {...props}/>
  )
}
TableData.propTypes = {
