...
export default class Authorities extends Component {
  ...
  onSelectedChange = (value) => {
    ...
    actions.fetchData(userAuthUrl, 1, searchId, name, value)
  }

  componentDidMount() {
    const {actions} = this.props
    //调用action获取数据
    actions.fetchData(userAuthUrl, 1)
  }

  onSearch = () => {
    const {searchId, name, selected} = this.state
    const {actions} = this.props
    actions.fetchData(userAuthUrl, 1, searchId, name, selected)
  }
  ...
  render() {
    const {authorities, actions} = this.props
    return (
      <div className="ant-layout-content" style={{marginTop: '20px'}}>
        ...
        {/* -- 显示组 start -- */}
        <TableData data={authorities.items}
