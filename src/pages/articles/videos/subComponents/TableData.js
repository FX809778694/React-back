...
import {ARTICLE_VIDEO} from '../../../../utils/routePath'
...
export default function TableData ({searchText, handleDelete, ...props}) {
  ...
  const columns = [
    {
      ...
      key: 'id',
      render: (text, record) => (
        <Link to={`${ARTICLE_VIDEO}/id/${record.id}`}>{text}</Link>
      ),
    }, {
      title: '视频标题',
      dataIndex: 'title',
      ...
      render: (text, record) => (
        <span style={{WebkitBoxOrient: 'vertical'}}>
          <Link to={`${ARTICLE_VIDEO}/id/${record.id}`}>{
    ...
    , {
      ...
      render: (text, record) => (
        <span style={{fontSize: 16}}>
          <Link to={`${ARTICLE_VIDEO}/${record.post.topicId}/subTable`}>
            <Icon type="copy" title="二级列表"/>
          </Link>
          <span className="ant-divider"/>
          <Link to={`${ARTICLE_VIDEO}/modify/${record.id}`}>
  ...
  return (
    <TableComponent label="视频列表" addLink={`${ARTICLE_VIDEO}/add`} columns={columns} {...props}/>
  )
}
TableData.propTypes = {
