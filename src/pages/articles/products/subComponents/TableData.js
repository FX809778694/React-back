...
import {ARTICLE_PRODUCT} from '../../../../utils/routePath'
...
export default function TableData ({searchText, handleDelete, ...props}) {
  ...
  const columns = [
    {
      ...
      key: 'id',
      render: (text, record) => (
        <Link to={`${ARTICLE_PRODUCT}/id/${record.id}`}>{text}</Link>
      ),
    }, {
      title: '产品名',
      dataIndex: 'title',
      ...
      width: '30%',
      render: (text, record) => (
        <Link to={`${ARTICLE_PRODUCT}/id/${record.id}`}>{
      ...
      render: (text, record) => (
        <span style={{fontSize: 16}}>
          <Link to={`${ARTICLE_PRODUCT}/${record.post.topicId}/subTable`}>
            <Icon type="copy" title="二级列表"/>
          </Link>
          <span className="ant-divider"/>
          <Link to={`${ARTICLE_PRODUCT}/modify/${record.id}`}>
            <Icon type="edit" title="修改原文"/>
          </Link>
          <span className="ant-divider"/>
          <a>
  ...
  return (
    <TableComponent label="产品列表" addLink={`${ARTICLE_PRODUCT}/add`} columns={columns} {...props}/>
  )
}
TableData.propTypes = {
