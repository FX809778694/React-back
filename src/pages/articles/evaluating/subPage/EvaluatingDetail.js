...
class EvaluatingDetail extends React.Component {
  constructor() {
    super();
    this.state = {
      loading: true
      ...
      <Card loading={loading} title={data.post && data.post.title} bordered={true}>
        { data &&
          <Form layout={'horizontal'}>
            ...
            <Items label="评测标题：" data={data.post && data.post.title}/>
            <Items label="评测描述：" data={data.post && data.post.description}/>
            ...
            <Items label="评测内容：" data={data.post &&
            ...
            <Items label="评测属性：" data={data.post &&
...
export default connect(mapStateToProps, mapDispatchToProps)(EvaluatingDetail)
