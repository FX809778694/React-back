...
import {ARTICLE_EVALUATE} from '../../../../utils/routePath'
...
export default function TableData ({searchText, handleDelete, ...props}) {
  ...
  const columns = [
    {
      ...
      key: 'id',
      render: (text, record) => (
        <Link to={`${ARTICLE_EVALUATE}/id/${record.id}`}>{text}</Link>
      ),
    }, {
      title: '评测标题',
      dataIndex: 'title',
      ...
      render: (text, record) => (
        <span style={{WebkitBoxOrient: 'vertical'}}>
          <Link to={`${ARTICLE_EVALUATE}/id/${record.id}`}>{
      ...
      render: (text, record) => (
        <span style={{fontSize: 16}}>
          <Link to={`${ARTICLE_EVALUATE}/${record.post.topicId}/subTable`}>
            <Icon type="copy" title="二级列表"/>
          </Link>
          <span className="ant-divider"/>
          <Link to={`${ARTICLE_EVALUATE}/modify/${record.id}`}>
  ...
  return (
    <TableComponent label="评测列表" addLink={`${ARTICLE_EVALUATE}/add`} columns={columns} {...props}/>
  )
}
TableData.propTypes = {
