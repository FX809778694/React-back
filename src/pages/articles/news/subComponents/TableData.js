...
import {ARTICLE_NEWS} from '../../../../utils/routePath'
...
export default function TableData ({searchText, handleDelete, ...props}) {
  ...
  const columns = [
    {
      ...
      key: 'id',
      render: (text, record) => (
        <Link to={`${ARTICLE_NEWS}/id/${record.id}`}>{text}</Link>
      ),
    }, {
      title: '新闻标题',
      dataIndex: 'title',
      ...
      render: (text, record) => (
        <span style={{WebkitBoxOrient: 'vertical'}}>
          <Link to={`${ARTICLE_NEWS}/id/${record.id}`}>{
      ...
      render: (text, record) => (
        <span style={{fontSize: 16}}>
          <Link to={`${ARTICLE_NEWS}/${record.post.topicId}/subTable`}>
            <Icon type="copy" title="二级列表"/>
          </Link>
          <span className="ant-divider"/>
          <Link to={`${ARTICLE_NEWS}/modify/${record.id}`}>
  ...
  return (
    <TableComponent label="新闻列表" addLink={`${ARTICLE_NEWS}/add`} columns={columns} {...props}/>
  )
}
TableData.propTypes = {
