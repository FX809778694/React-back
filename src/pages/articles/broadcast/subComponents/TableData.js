...
import {ARTICLE_BROADCAST} from '../../../../utils/routePath'
...
export default function TableData ({searchText, handleDelete, ...props}) {
  ...
  const columns = [
    {
      ...
      key: 'id',
      render: (text, record) => (
        <Link to={`${ARTICLE_BROADCAST}/id/${record.id}`}>{text}</Link>
      ),
    }, {
      title: '直播标题',
      dataIndex: 'title',
      ...
      render: (text, record) => (
        <span style={{WebkitBoxOrient: 'vertical'}}>
          <Link to={`${ARTICLE_BROADCAST}/id/${record.id}`}>{
      ...
      render: (text, record) => (
        <span style={{fontSize: 16}}>
          <Link to={`${ARTICLE_BROADCAST}/${record.post.topicId}/subTable`}>
            <Icon type="copy" title="二级列表"/>
          </Link>
          <span className="ant-divider"/>
          <Link to={`${ARTICLE_BROADCAST}/modify/${record.id}`}>
  ...
  return (
    <TableComponent label="直播列表" addLink={`${ARTICLE_BROADCAST}/add`} columns={columns} {...props}/>
  )
}
TableData.propTypes = {
